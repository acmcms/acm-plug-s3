...
class ChangeForHistory extends ChangeAbstract {
	...
	ChangeForHistory(final StorageLevel3 storage, final EntryImpl entry, final String historyId) {
	...
	public void commit() {
		...
		try {
			...
			if (!removed.baseHasKeysOwn() && !added.baseHasKeysOwn()) {
				transaction.revert(this.entry.getLink(), this.historyId, folder, created, state, title, typeName);
			} else {
				transaction.revert(this.entry.getLink(), this.historyId, folder, created, state, title, typeName, removed, added);
			}
