...
final class InvalidatorRetry implements Runnable {
	...
	public final void run() {
		...
		this.delay *= 2;
		if (this.delay <= 32_000L) {
			Report.info(
					"S3-INV-RETRY",
					"Extra invalidate fail - will retry again: guid=" + this.target.getName() + ", life="
							+ Format.Compact.toPeriod(System.currentTimeMillis() - this.target.lastModified()) + ", size=" + Format.Compact.toBytes(this.target.length()));
