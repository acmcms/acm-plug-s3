...
class FieldsReadableTypeOverride extends BaseHostSealed implements BaseProperty {
	...
	FieldsReadableTypeOverride(final StorageLevel3 storage, final LinkData link, final String typeOverride) {
	...
	public BaseObject propertyGet(final BaseObject instance, final String key) {
		...
		switch (key.length()) {
			...
			case 5 :
				if (key.charAt(0) == '$') {
					if ("$type".equals(key)) {
						return Base.forString(this.typeOverride);
					}
					if ("$guid".equals(key)) {
	...
	public String toString() {
		...
		return "[object " + this.baseClass() + "(" + "type=" + this.typeOverride + ")]";
	}
